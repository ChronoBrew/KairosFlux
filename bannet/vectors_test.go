...
// BANLV 协议跨语言测试向量的 Go 侧锚点。加载 docs/banlv/vectors.json（由本包
// bannet.DataPack 生成，见 docs/BANLV-协议规范.md 附录与该文件生成时用的
// 一次性脚本说明），验证 bannet.DataPack.Pack 对每条向量都能重新生成完全
// 相同的字节序列。
//
...
func loadVectors(t *testing.T) []protocolVector {
	t.Helper()
	path := filepath.Join("..", "docs", "banlv", "vectors.json")
...
// 都能重新生成与冻结样本完全一致的整帧字节——锁定 BANLV 帧编码不被意外改动。
func TestVectors_FramePackMatchesGolden(t *testing.T) {
	for _, v := range loadVectors(t) {
		...
		t.Run(v.Name, func(t *testing.T) {
			...
			gotFrame, err := bannet.NewDataPack().Pack(bannet.NewMessage(v.MsgID, data))
			if err != nil {
...
// docs/BANLV-协议规范.md 记录这是 v1 的已知限制，本测试确保该假设不被静默打破。
func TestVectors_FrameHeadLenIsSixBytes(t *testing.T) {
	if got := bannet.NewDataPack().HeadLen(); got != 6 {
...
// 投递/retention 机制是否需要改动，见 docs/BANLV-协议规范.md 与 service/
