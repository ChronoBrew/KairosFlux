...
import "github.com/NeverENG/BanDB/bannet/transport"

// 本文件是重构第五步、也是最后一步（拆 transport 包，见
// docs/rfc/bannet-重构.md C.7 步骤 5）的门面：Connection/ConnManager/
// ConnRegistry 的实现已经搬进 bannet/transport，这里用类型别名（非包装
// 类型）把根包的公开标识符原样保留——已有调用方与测试（本包内
// lifecycle_convergence_test.go 里对 *bannet.Connection 的类型断言等）
// 不需要改一行代码或 import 路径。
//
...
// Connection 是传输层的连接实现，参见 bannet/transport.Connection。
type Connection = transport.Connection

// ConnRegistry 是连接注册表的契约，参见 bannet/transport.ConnRegistry。
type ConnRegistry = transport.ConnRegistry

// ConnManager 是 ConnRegistry 的默认实现，参见 bannet/transport.ConnManager。
type ConnManager = transport.ConnManager

// NewConnManager 构造一个 ConnManager，转发到 bannet/transport.NewConnManager。
