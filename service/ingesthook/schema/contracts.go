...
//     （QuantScout=Python、QuantBrew=Rust）在各自接入 KairosFlux 契约时补齐，本次不
//     抢跑。
//   - 字段级别的校验规则（必填/OHLC一致性/涨跌幅上限）仍然是 Go 代码
//     （QuoteSnapshot.Validate），不是从 JSON 数据驱动出来的通用规则引擎——RFC
//     §9.4 明确这是渐进式迁移，KairosFlux CLAUDE.md 的"外科手术式修改"也不支持为了
...
// 强制校验"的一部分（方案 M1 任务 2），失败必须让调用方（cmd/kairosflux-server 的
