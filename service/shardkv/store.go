...
//
// v1 边界（诚实标注）：
//   - 拓扑：所有节点托管所有分片 → 这是「多组副本」而非「数据跨节点分区」；与 network 层
//     #191 的一致性哈希分区转发（key→属主节点、无副本）是两套不同一致性模型，ShardKV 自成
//     一层、不劫持 kv.Write。真正的按分片分区放置（shard→节点子集）留后续。
//   - 存储：每分片 FSM 用内存 store（下面的 KVStore 接口），刻意避开「每分片存储隔离 + 全局
//     配置」难点——把真 LSM 引擎插到 KVStore 边界是独立后续。
//   - 读：本地副本读是最终一致（apply 异步）；线性一致的 leader 读/read-index 留后续。
package shardkv
