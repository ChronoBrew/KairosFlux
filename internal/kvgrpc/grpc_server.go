// Package kvgrpc 是 KairosFlux 的 gRPC 传输实现，位于 internal/ 之下：它是内部传输，
// 不是对外契约。
//
// 定位（务必读完再接线到生产）：gRPC 在本项目里是基准测试/协议对照用途，不是
// 生产摄入入口——生产摄入走 kairnet 自研 TLV 协议（Kair，见
// docs/Kair-协议规范.md）。实测显示 Kair 在不受 fsync 约束的读路径上吞吐约为
// gRPC 的 2.7 倍（写路径两者持平，受同一 fsync 瓶颈约束，与入口协议无关），这是
// 自研协议存在的理由、不是历史包袱，详见 README.md 的性能一节；cmd/kairosflux-grpc-server 与
// cmd/kairosflux-bench-grpc 的存在是为了给 Kair 提供一个可对照的性能/协议基线，
// 不代表它是与 kairnet 并列的生产候选。
//
// 对外契约只有两样：client 包（Go SDK）与 Kair 协议规范（docs/Kair-协议规范.md，
// 服务端实现见 kairnet/）。把 .proto 交给使用方自行生成客户端，等于把内部传输
// 实现当成公开 API——使用方要装 protoc 工具链、要理解 protobuf，还要跟着我们的
// proto 变更走。需要多语言接入 Kair 时，走各语言的最小 TLV 客户端（如
...
// 见 cmd/kairosflux-grpc-server/main.go 的构造处。
func NewGRPCServer(kv *service.KVServer, filter *ingesthook.Filter) *GRPCServer {
	return &GRPCServer{kv: kv, filter: filter}
}

// Put 写入前先经 Filter.Validate 清洗——与 kairnet 入口（service/ingesthook.Filter.Handle）
// 复用同一套规则，只是跳过 kairnet 特有的帧解析（gRPC 已经是结构化的 Key/Value，无需
