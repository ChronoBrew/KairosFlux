...
在建连后选定，搭载在 §5 已经定义的 v2 HELLO 请求/响应负载里传递——§5 本身
没有规定这个负载的内部字段格式（只定出了"客户端支持的版本"这一项，具体
格式留到定稿阶段），本节只是在这个本就未定稿的负载里追加"客户端期望的 ack
档位"这一项协商内容，不是在 §5 的握手流程之外另开一条协商通道，也不改变
§5 描述的"发 HELLO、按超时判断对端版本"这个流程本身。字段的具体格式与 §5
的 `payload` 一样留到定稿阶段，不在本节展开——这不影响本节的核心设计，
档位选定后的行为才是本节的重点。
