...
> 作者裁决：Kair v2 开发放行，场景裁决为"写多读少、甚至可以不回调"——真实
...
> `kairnet/`、`client/`、`docs/Kair-协议规范.md` 里描述的任何 v1 行为——v1 是且
...
Kair v1 一直"够用"，但 QuantScout 用 5241 行真实行情做全量实测这一轮，暴露了
...
1. **响应体没有结构化的错误细节**（对应 D2，见
   ...
   （见 `docs/Kair-协议规范.md` §3.4）——能用，但只是把本该在协议设计阶段
   就有的结构，事后拿协议扩展的方式补回来。
...
`docs/Kair-协议规范.md` §2）——v2 第一次真正给 `HELLO` 赋予行为（版本协商），
`BYE` 仍然保留但本轮不定义具体语义。
...
`docs/Kair-协议规范.md` §3.4）。v2 把这个补丁转正为正式设计：
...
- v2 客户端**不能**用"阻塞等 HELLO 响应"的方式做协商——会在 v1 服务端上
  ...
  超时即判定为 v1，**不视为错误**。
- 复用同一条连接可行的前提是：v1 服务端的读循环（`kairnet/connection.go` 的
...
实现阶段回写的文档改动（第一阶段实现见 `kairnet/negotiate` 包，向量见
`docs/kair/vectors-v2.json` 的 `negotiation` 分区）。`§11.2` 提到的"字段
...
- `version`：客户端支持的最高 Kair 版本号，本阶段固定写 `0x02`。
...
- **流水线（`corr_id`）是否真的做**：**已裁决：做，但只做窗口语义，不做
  ...
  做"的悬而未决变成了确定输入——Kair 的实际负载正是量化数据批量写入
...
- **`flags` 的压缩/流式位**：**部分裁决**。Triple 用 HTTP/2 的 `END_STREAM`
  ...
  若 Kair v2 未来真的做流式，`flags` 应至少留一位给等价的"流结束"语义
...
- **`payload` 的结构化格式**：**已裁决方向，细节未定**。Triple 对"错误"与
  ...
  勉强统一（见调研 §4、§8）。Kair v2 采纳同样的取向：`payload`（正常返回
...
`docs/kair/vectors.json` 当前的 12 条向量全部是 v1 格式，是 Go/Python 两侧
实现共同验证的锚点（见 `docs/Kair-协议规范.md` §6）。v2 定稿后的迁移方案
（草案）：

1. **不修改现有 12 条向量**——它们验证的是 v1 帧格式，v1 协议本身不变，这些
   向量长期有效。
2. **新增 `docs/kair/vectors-v2.json`**，独立的向量文件，覆盖 v2 帧格式
   （新头部布局、数字 opcode、`type` 分派、HELLO 协商的请求/响应对）。不
   合并进同一个文件，是因为 v1/v2 是两套完全不同的字节布局，混在一起容易
   在生成脚本里出错、也不便按协议版本分别审阅。
3. **新增 `docs/kair/vectors-negotiation.json`**（或作为 vectors-v2.json 的
   一个分区）：专门覆盖 §5 协商流程的边界情况——v2 客户端连 v1 服务端超时
   降级、v2 客户端连 v2 服务端协商成功两条路径都要有向量级别的回归锁定，
   不能只在集成测试里测，因为协商失败的后果（连接卡死）比帧解析错误更隐蔽。
4. **Go/Python 两侧的向量测试文件各自新增**（`kairnet/vectors_v2_test.go`、
...
v1.1 的 `reason` 字段（`docs/Kair-协议规范.md` §3.4）解决了"客户端知道
...
不凭空设计一套分类，直接对齐这轮 kairnet 健壮性审计（见
...
`proto.StatusOK`/`StatusNotFound` 等 v1 状态字符串（`docs/Kair-协议规范.md`
...
2026-08-20 作者裁决明确了 Kair v2 要适配的真实负载：**写多读少，甚至可以
不回调**。真实例子是 QuantScout 每日批量导出 5241 行全市场行情快照全量写入
KairosFlux（`docs/iteration-2026-08-20-quantscout-realdata-fixes.md:1,8`）——客户端
灌数据进来，大多数时候不需要逐条等响应。

这个场景与 v1/v2 现有设计的默认假设正面冲突。已用代码核实（不是猜测）：

- **v1 是严格的"发一帧、收一帧"协议**：`docs/Kair-协议规范.md:159` 明文
...
响应，造成整条连接的响应错位"（`docs/Kair-协议规范.md:161`）。`ack=window`
...
如果连接在客户端能发出 `STAT`/`BYE` 之前就整体中断——例如 `docs/Kair-
...
`ingesthook.Filter.Handle(req kairnet.Request) (kairnet.HookAction, string)`
（`service/ingesthook/filter.go:64`）收到的 `kairnet.Request` 本身已经带着
连接身份——`Request.Conn()` 返回的 `Conn` 接口自带 `ID() uint32` 与
`SetProperty`/`Property`/`RemoveProperty`（`kairnet/handler/handler.go:50-56`）
...
的话下限是什么"，没有回答"Kair 该不该做、做到多满"。2026-08-20 的场景
...
- **不做的部分**：真正的流水线要求响应可以乱序返回、`corr_id` 要能把
  乱序到达的响应重新对回请求——Kair v2 不做这件事，窗口按 FIFO 顺序关闭
