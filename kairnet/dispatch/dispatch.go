...
func (m *MsgHandle) AddRouter(msgID string, r handler.Handler) {
	if _, ok := m.routers[msgID]; ok {
		slog.Warn("kairnet duplicate route registration ignored", "msgID", msgID)
		return
	}
	m.routers[msgID] = r
}
...
func (m *MsgHandle) DoMsgHandle(request handler.Request) {
	defer func() {
		if r := recover(); r != nil {
			...
			slog.Error("kairnet handler panicked, recovered", "msgID", request.MsgID(),
	...
	if !ok {
		slog.Error("kairnet unregistered msgID", "msgID", request.MsgID())
		return
	}
...
func (m *MsgHandle) StartOneWorker(workerID int, taskQueue chan handler.Request) {
	...
	slog.Debug("kairnet worker started", "workerID", workerID)
	for {
		select {
		...
		case <-m.stopCh:
			...
			for {
				...
				slog.Debug("kairnet worker exited", "workerID", workerID)
				return
			}
		}
	}
...
func (m *MsgHandle) Stop() {
	slog.Debug("kairnet worker pool stopping")
	close(m.stopCh)
	...
	select {
	case <-done:
		slog.Debug("kairnet worker pool drained")
	case <-time.After(stopDrainTimeout):
		slog.Warn("kairnet worker pool did not drain within timeout, continuing shutdown anyway")
	}
}
