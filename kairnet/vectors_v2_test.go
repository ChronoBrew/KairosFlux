...
// Kair v2 协议跨语言测试向量的 Go 侧锚点，与 vectors_test.go（v1）并存不合并——
// 见 docs/rfc/Kair-2.md §8 迁移方案第 4 条："两套协议版本的测试意图不同，合并
// 会让测试文件同时承担'锁定 v1 不变'与'验证 v2 新增'两个不同职责"。
//
// 加载 docs/kair/vectors-v2.json（手工推导生成，不是本实现自己 Pack 出来再
...
func loadVectorsV2(t *testing.T) vectorsV2File {
	t.Helper()
	path := filepath.Join("..", "docs", "kair", "vectors-v2.json")
...
// docs/kair/vectors-v2.json 该向量的 description。
