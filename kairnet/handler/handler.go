// Package handler 是 kairnet 面向业务代码的契约层：用户实际要实现/使用的
// 唯一一组类型（Handler/Request/HookAction/Conn），边界不因本次重构而
// 变复杂——这是重构 RFC（docs/rfc/bannet-重构.md）C.2 所说的"业务层"。
//
// 本包不依赖 kairnet 的任何其它子包（codec/lifecycle/transport/dispatch），
...
//
// 该偏差已记录在迭代文档里；对外行为无影响：根包 kairnet.Conn 仍是
...
type Conn interface {
	...
	// 错误的字节）。Kair v2 响应（RFC docs/rfc/Kair-2.md §2/§3/§11）与 §5
...
// 具体实现（dispatch 包的 request 结构体，原 kairnet/request.go）在 dispatch，
...
// RequestV2 是一次分派给 HandlerV2 的 Kair v2 请求单元（RFC
// docs/rfc/Kair-2.md §2/§3）：已解码的 v2 帧字段 + 发出该帧的连接引用。
...
// HandlerV2 是处理 Kair v2 帧的业务契约：单一方法内部按 Opcode 自行分派
