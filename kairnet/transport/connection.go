...
func recoverConnGoroutine(connID uint32, where string, lc *lifecycle.Lifecycle) {
	if r := recover(); r != nil {
		...
		slog.Error("kairnet connection goroutine panicked, recovered",
...
type Connection struct {
	...
	// kairnet，持有 dispatch.MsgHandle）决定怎么分派——这正是 transport 不
	...
	// NegotiateFunc 是 Kair v2 协商的接入点（docs/rfc/Kair-2.md §5/§5.1），
	...
	// kairnet/lifecycle 包顶部注释里 Draining（决定关闭但写路径仍打开）与
...
// negotiateFunc/onFrameV2 是 Kair v2 协商/分派的接入点（见各自字段的
...
func (c *Connection) StartReader() {
	...
	// usingV2 一旦置 true 永不回退——Kair v2 协商（RFC §5）是一次性的、
	...
	for {
		if !usingV2 {
			...
			// Kair v2 协商接入点（docs/rfc/Kair-2.md §5/§5.1）：在把这一帧
...
var errConnClosed = errors.New("kairnet: connection closed")
...
func (c *Connection) SendMsg(msgID string, data []byte) error {
	...
	// kairnet/lifecycle 包顶部关于 Draining 与 Done 的注释。
	select {
...
// payload 重新打包）。Kair v2 的所有响应（含协商响应本身，见 StartReader
