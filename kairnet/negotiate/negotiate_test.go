...
// negotiate 包的协商集成测试，覆盖 docs/rfc/Kair-2.md §5 描述的两条路径。
//
// 第二阶段更新（生产接线落地后）：kairnet.Server 的连接读循环现在无条件
// 识别 §5.1 的 HELLO 探测帧并回复 v2 响应（见 kairnet/server.go 的
// onNegotiate/transport.Connection.NegotiateFunc）——这不是"v2 能力
// 打开时才生效"的可选项，任何用 kairnet.NewServer() 起的服务端（哪怕像
...
//     kairnet.Server（含 service.Router，v1 PUT/GET/DEL 路由照常注册），
...
//     互相说通，与 1 的区别是这里不依赖 kairnet.Server 的生产接线。
//
...
// StartReader 不应该把探测帧转交给 OnFrame（见 kairnet/server.go 的
...
func (h *helloTrapRouter) PreHandle(req kairnet.Request) kairnet.HookAction {
	return kairnet.HookPass
}
func (h *helloTrapRouter) Handle(req kairnet.Request) {
	h.t.Fatalf("HELLO 探测帧不应该被分派到业务路由（本应在 transport 层被 NegotiateFunc 拦截）")
}
func (h *helloTrapRouter) PostHandle(req kairnet.Request) {}
...
// 这个服务端对 HELLO 探测帧的响应完全由 kairnet.Server 内部的 onNegotiate
// 给出，不依赖这里注册的任何路由。
...
// 确认"：对一个真实的、按生产接线组装的服务端（kairnet.Server + service.
