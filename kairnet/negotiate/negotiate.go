// Package negotiate 实现 Kair v1/v2 的 HELLO 协商（docs/rfc/Kair-2.md
// §5/§5.1）：v2 客户端连接后先发一个 v1 格式的 HELLO 帧，按是否在超时内
// 收到 v2 格式的响应判断对端版本，零破坏地与 v1 共存于同一条连接。
//
// 本包是新增的构建块，不修改 kairnet/transport、service/router.go 等任何
// 现有生产路径——真实的服务端/客户端接线（把这里的能力接进
// kairnet.Server/client.Client）是后续阶段的工作，本阶段只交付协商逻辑本身
// 并用真实的 v1 生产服务端（service.Router）与本包提供的最小 v2 响应
// 函数验证它确实可用，而不是假设可用。
//
// 依赖 kairnet/codec 里的 v1 DataPack（编码探测帧要用 v1 格式，见 §5.1）与
// v2 DataPackV2（解码/编码响应要用 v2 格式），以及 proto 的 MsgHello 常量
// ——这也是本包与 kairnet/codec 分开的原因：codec 包的既有定位是"只认字节，
// 不认 msgID 该分派给谁"（见 kairnet/codec/message.go 包注释），而协商逻辑
...
// Version 是协商结果：对端实际使用的 Kair 协议版本。
type Version uint8
...
// 定义这个字符串常量而漂移。
const ConnPropertyAckTier = "kair.v2.ackTier"
