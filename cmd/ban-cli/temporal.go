...
// ban-cli 对时态内核 M0 新增四个 v2 opcode（PUT_VERSIONED/GET_AS_OF/
...
// opcode，没有 v2 能力；这里不去扩建一整套 v2 SDK，只用 bannet/negotiate 与
// bannet/codec 已经导出的协商/拼帧函数拼出"连一次、发一帧、收一帧"的最小
...
// 工具：本仓库已有 cmd/ban-ingest 那种直接 storage.NewEngine 的离线模式先例，
...
func runTemporalCommand(addr string, args []string) int {
	...
	case "put-versioned":
		if len(args) < 3 {
			fmt.Fprintln(os.Stderr, "用法: ban-cli put-versioned <key> <val>")
			return 2
		}
	...
	case "get-as-of":
		if len(args) < 3 {
			fmt.Fprintln(os.Stderr, "用法: ban-cli get-as-of <key> <as_of_nanos>")
			return 2
		}
	...
	case "list-versions":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "用法: ban-cli list-versions <key>")
			return 2
		}
	...
	case "fingerprint":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "用法: ban-cli fingerprint <prefix>")
			return 2
		}
...
// 累计记账（见 bannet/codec.OpcodePutVersioned 的文档），用 every 语义天然
// 匹配。
type v2Conn struct {
	conn net.Conn
}
